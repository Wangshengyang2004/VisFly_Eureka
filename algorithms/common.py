...
def compute_td_returns(r, done, next_value, episode_done=None, gamma=0.99, lamda=0.95):
    ...
    returns = [th.zeros(num_envs, dtype=th.float32, device=r[0].device) for _ in range(h)]  # 初始化返回值列表
    episode_done = done if episode_done is None else episode_done

    # 初始化 Ai、Bi 和 lam，确保所有张量都是float32
    ...
    # 确保next_value和其他张量都是float32
    next_value_f32 = [nv.to(dtype=th.float32) if nv is not None else None for nv in next_value]
    r_f32 = [r_t.to(dtype=th.float32) for r_t in r]
    done_f32 = [d.to(dtype=th.float32) for d in done]
    episode_done_f32 = [ed.to(dtype=th.float32) for ed in episode_done]
    
    Bi = next_value_f32[-1] * (~done_f32[-1].bool())
    # 反向循环计算 TD-λ 返回值
    for t in reversed(range(h)):
        active = (~done_f32[t].bool()).float()  # 未终止的环境掩码，1 表示未终止，0 表示已终止
        done_mask = done_f32[t]  # 终止的环境掩码，1 表示已终止，0 表示未终止
        episode_active = (~episode_done_f32[t].bool()).float()
        # 更新 lam，对于已终止的环境，将 lam 重置为 1
        lam = lam * lamda * active + done_mask

        # 更新 Ai
        Ai = active * (
                lamda * gamma * Ai + gamma * next_value_f32[t] + ((1.0 - lam) / (1.0 - lamda)) * r_f32[t]
        )

        # 更新 Bi
        Bi = gamma * (next_value_f32[t] * done_mask * episode_active + Bi * active) + r_f32[t]
