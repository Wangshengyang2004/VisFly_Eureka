"""
Training utilities for VisFly-Eureka.

Provides functions for training VisFly environments with injected reward functions
using BPTT (differentiable simulation) and PPO algorithms.
"""
...
def train_with_generated_reward(
    ...
    try:
        if algorithm.lower() == "bptt":
            return _train_with_bptt(env, steps, device, save_path, log_dir)
        elif algorithm.lower() == "ppo":
            return _train_with_ppo(env, steps, device, save_path, log_dir)
        else:
...
def _train_with_bptt(
    ...
    try:
        ...
        alg_config_path = config_root / "algs" / "navigation" / "bptt.yaml"
        with open(alg_config_path, "r") as f:
            alg_config = yaml.safe_load(f)

        # Use algorithm config directly, only add required runtime parameters
        bptt_params = alg_config["algorithm"].copy()
        bptt_params["env"] = env
        if save_path:
            bptt_params["save_path"] = save_path
        if log_dir:
            bptt_params["tensorboard_log"] = log_dir

        model = BPTT(**bptt_params)

        logger.debug(f"Starting BPTT training for {steps} steps")

        # Train the model - BPTT handles training internally
        model.learn(total_timesteps=steps)

        logger.debug("BPTT training completed successfully")
        return model
...
def _train_with_ppo(
    ...
    try:
        # Import PPO (fallback to stable-baselines3 if VisFly version not available)
        try:
        ...
        logger.debug(f"Starting PPO training for {steps} steps")

        # Train the model
        model.learn(total_timesteps=steps)

        logger.debug("PPO training completed successfully")
        return model
...
def evaluate_model_performance(
    model: Any, env: Any, num_episodes: int = 10, max_episode_steps: int = 256
) -> TrainingResult:
    """
    ...
    logger.debug(
        f"Evaluation complete: {success_rate:.3f} success rate, {avg_episode_length:.1f} avg steps"
    )
    return result
...
def safe_model_training(
    env: Any, reward_code: str, algorithm: str, steps: int
) -> Optional[Any]:
    """
...
class TrainingMonitor:
    ...
    def get_average_performance(self) -> float:
        ...
        if not self.training_history:
            return 0.0
        return sum(r.score() for r in self.training_history) / len(
            self.training_history
        )
