...
class ParallelTrainingManager:
    ...
    def __init__(self, results_dir: str = "results", max_workers: int = 4, gpu_resource_manager: Optional[DynamicGPUResourceManager] = None):
        """
    ...
    def save_reward_function(self, reward_code: str, job_id: str, metadata: Optional[Dict] = None) -> str:
        """
    ...
    def save_injected_code(self, injected_code: str, job_id: str, injection_result: bool) -> str:
        """
    ...
    def submit_job(self, job: TrainingJob) -> bool:
        ...
        device_id = self.gpu_resource_manager.allocate_gpu_with_queue(
            job.job_id, 
            job.memory_requirement_mb, 
            allow_cpu_fallback=True
        )
        
        if device_id is None:
            self.logger.warning(f"Cannot submit job {job.job_id} - no device available, added to queue")
            return False
        ...
        if device_id == "cpu":
            job.environment_vars["CUDA_VISIBLE_DEVICES"] = ""  # Hide all GPUs for CPU-only execution
        ...
        try:
            ...
            with open(stdout_file, 'w') as stdout, open(stderr_file, 'w') as stderr:
                ...
                try:
                    ...
                    self.logger.warning(f"Job {job.job_id} timed out after {job.max_runtime_seconds}s")
                    process.kill()
                    exit_code = -1
                finally:
            ...
            if "CUDA out of memory" in stderr_content or "OutOfMemoryError" in stderr_content:
                should_retry = self.gpu_resource_manager.handle_cuda_oom_error(job.job_id)
                if should_retry:
            ...
            self.logger.info(f"Job {job.job_id} completed: {'SUCCESS' if success else 'FAILED'} "
                           f"in {runtime:.1f}s")
    ...
    def run_parallel_jobs(self, jobs: List[TrainingJob], max_concurrent: Optional[int] = None) -> Dict[str, JobResult]:
        """
        ...
        self.logger.info(f"Running {len(submitted_jobs)} jobs in parallel (max {max_concurrent} concurrent)")
    ...
    def _save_session_summary(self, results: Dict[str, JobResult]):
        ...
        self.logger.info(f"Session results: {summary['successful_jobs']}/{summary['total_jobs']} jobs succeeded")
    ...
    def _monitor_job_memory(self, job_id: str, process: subprocess.Popen):
        ...
        while process.poll() is None:  # While process is running
            try:
                ...
                stdout_file = Path(self.active_jobs[job_id] if job_id in self.active_jobs else "").parent / "stdout.log"
                                    ...
                                    for part in parts:
                                        if "/" in part:
                                            ...
                                            if step_num > training_step:
                                                ...
                                                self.gpu_resource_manager.update_job_memory_usage(job_id, training_step)
                                                break
