...
def parse_args():
    ...
    parser.add_argument('--env', '-e', type=str, default='navigation',
                        choices=['navigation', 'hover', 'object_tracking', 'tracking', 'catch', 'landing', 'racing', 'flip', 'vis_landing'],
    ...
    parser.add_argument('--weight', '-w', type=str, default=None,
                        help='Model weights file for testing')
    ...
    if reward_function_path and os.path.exists(reward_function_path):
        try:
            ...
            if 'get_reward' in exec_globals:
                ...
                print(f"✅ Successfully injected reward function from {reward_function_path}")
                return True
            else:
...
def create_environment(env_name: str, env_config: dict, is_training: bool = True):
    ...
    if 'target' in env_config[config_key] and isinstance(env_config[config_key]['target'], list):
        env_config[config_key]['target'] = th.tensor([env_config[config_key]['target']])
    
    env = env_class(**env_config[config_key])
...
def run_testing(args):
    ...
    try:
        ...
        print(f"Test Results: {steps} steps, avg reward: {avg_reward:.4f}, total: {total_reward:.4f}")
        ...
        with open(test_folder / f"test_results_{args.comment or 'default'}.json", 'w') as f:
